...
//SHMZ
if (file_exists("modules/grammar/functions.inc.php")) {
    include_once("modules/grammar/functions.inc.php");		// for using grammar function to retreive grammar settings
    };
if (file_exists("modules/xactview/functions.inc.php")) {
    include_once("modules/xactview/functions.inc.php");		// for using xactview functions to retreive xactview settings
    };
if (file_exists("modules/xmpp/functions.inc.php")) {
    include_once("modules/xmpp/functions.inc.php");         // for using xmpp functions to retreive xmpp settings
    };
...
//SHMZ
if (function_exists("grammar_get") && function_exists("grammar_update") && function_exists("grammar_del")) {
	$grammar_exists = TRUE;
} else {
...
function exportextensions_allusers() {
	...
	foreach ($exts as $ext) {
		...
		//SHMZ		
		/* Obtain grammar settings */ 
		if($grammar_exists) {
			$grammar_settings = grammar_get("user", $e);
			$grammar_vmx_grammar0 = grammar_get("vmx$e","0");
            $grammar_vmx_grammar1 = grammar_get("vmx$e","1");
            $grammar_vmx_grammar2 = grammar_get("vmx$e","2");
            $grammar_vmx_grammar_enabled = grammar_enabled("vmx$e","0");

		}
		if(isset($grammar_settings)) {
			$grammarvocab = isset($grammar_settings)?$grammar_settings:"";
			$vmx_grammar_enabled = isset($grammar_vmx_grammar_enabled["enabled"])?$grammar_vmx_grammar_enabled["enabled"]:"0";
			$vmx_grammar0 = isset($grammar_vmx_grammar0["vocabulary"])?$grammar_vmx_grammar0["vocabulary"]:'0\noperator';
			$vmx_grammar1 = isset($grammar_vmx_grammar1["vocabulary"])?$grammar_vmx_grammar1["vocabulary"]:'1\nfollow-me';
			$vmx_grammar2 = isset($grammar_vmx_grammar2["vocabulary"])?$grammar_vmx_grammar2["vocabulary"]:'2';
		}
		...
		/*SHMZ*/		
		$csv_line[$csvi++]	= isset($grammarvocab)?$grammarvocab:"";
		...
		$csv_line[$csvi++]  	= isset($xmpp_settings["jabber_user"])?$xmpp_settings["jabber_user"]:"";
		$csv_line[$csvi++]  	= isset($xmpp_settings["jabber_pass"])?$xmpp_settings["jabber_pass"]:"";
